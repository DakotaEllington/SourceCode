...
unsigned int nStakeTargetSpacing = .5 * 60; // 30 seconds block spacing
const int64 nChainStartTime = 1377190500; 
...
const string strMessageMagic = "ButterflyCoin Signed Message:\n";
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal ButterflyCoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ButterflyCoin
    bool fStrictPayToScriptHash = true; // Always active in ButterflyCoin
    ...
    // ppcoin: fees are not collected by miners as in ButterflyCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ButterflyCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Butterflycoin!";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:ButterflyCoin genesis block";
        ...
        block.nVersion = 1;
        block.nTime    = 1377190500;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1913603;
        ...
        assert(block.hashMerkleRoot == uint256("0xef2a7d44e0777624accd69541f050387dddaf12baaca8fba0ab768a2d83cd3ea"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("ButterflyCoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("ButterflyCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ButterflyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("ButterflyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadButterflyCoinMiner(void* parg);

static bool fGenerateButterflyCoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void ButterflyCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("ButterflyCoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateButterflyCoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateButterflyCoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running ButterflyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadButterflyCoinMiner(void* parg)
{
    ...
    try
    {
        ...
        ButterflyCoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadButterflyCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadButterflyCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadButterflyCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateButterflyCoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateButterflyCoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateButterflyCoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d ButterflyCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadButterflyCoinMiner, pwallet))
                printf("Error: NewThread(ThreadButterflyCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
